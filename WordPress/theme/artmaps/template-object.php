...
foreach(array(
                ...
                'jquery-ui-dialog', 'jquery-xcolor', 'json2', 'markerclusterer',
...
jQuery(document).ready(function($) {
    ...
    ArtMaps.UI.formatMetadata = function(metadata) {
        <?= $templateEngine->renderObjectMetadataFormatJS($blog) ?>
    };

    ArtMaps.UI.formatTitle = function(metadata) {
...
<div class="artmaps-action-comment-button">Comment</div>
...
foreach(get_approved_comments($post->ID) as $comment) {
    ...
    <a href="<?= $comment->comment_author_url ?>" target="_blank">[Link]</a><br />
    </div><?php
}
?>
</div>
